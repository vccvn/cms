...
class CrawlerFrame extends Model
{
    ...
    public $fillable = ['name', 'type', 'url', 'logo', 'index', 'selectors', 'deleted'];
    ...
    public function getLogo($urlencode=false)
    {
        ...
        $url = asset('static/crawlers/'.$logo);
    ...
    {
        if($this->logo && file_exists($path = public_path('static/crawlers/'.$this->logo))){
            unlink($path);
        }
    }
