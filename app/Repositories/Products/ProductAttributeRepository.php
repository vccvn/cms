...
class ProductAttributeRepository extends BaseRepository
{
     ...
     * @return bool
     */
    public function saveAttributeValues1($product_id, $category_id, array $attributes = [], array $attributeGroup = [])
    {
        $this->checkRepositories();
        // xóa thuộc tính dư thừa
        $this->deleteAttributeValueIgnoreList($product_id, array_merge($attributeGroup['use_list'], $attributeGroup['use_value']));
        ...
        if ($attributeGroup['attrs']) {
            foreach ($attributeGroup['attrs'] as $attribute) {
                ...
                if (!isset($attributes[$attribute->name])) {
                    $this->deleteAttributeValueIgnoreList($product_id, [$attribute->id]);
                } else {
     ...
     * @return array
     */
    public function saveAttributeValues($product_id, $category_id, array $attributes = [], array $attributeGroup = [], array $variants = [], array $variant_price = [], array $variant_values = [])
    {
        $this->checkRepositories();
        // xóa thuộc tính dư thừa
