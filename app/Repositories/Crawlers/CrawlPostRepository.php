...
class CrawlPostRepository extends PostRepository
{
    ...
    public function crawl(array $args = [], $frame = null)
    {
        ...
        try {
            ...
            $data = [
                'title' => html_entity_decode(strip_tags($this->getContent($html->find($frame->title, 0), $frame->title_attr))),
            ...
            if ($req->dynamic_id && $dynamic = $this->dynamics->findBy('id', $req->dynamic_id)) {
                ...
                // lưu file ảnh 
                $featureImage = @$this->saveFeatureImage($url, $qid, 'posts', 414, 276, 'thumbs');
                $data['feature_image'] = $featureImage;
                if ($featureImage && file_exists($fp = public_path('static/posts/' . $featureImage))) {
                    $image = new Image($fp);
                    ...
                    $image->save(public_path('static/posts/social/' . $featureImage));
                ...
                if ($post) {
                    ...
                    if ($req->crawl_resources) {
                        ...
                        $this->save(['content' => $this->saveResources($frame, $data['content'], $qid, 'posts')], $post->id);
                    }
