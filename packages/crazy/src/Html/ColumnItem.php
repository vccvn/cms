...
class ColumnItem{
    ...
    public static function render()
    {
        ...
        if($type == 'text' || $options->text){
            $content = e(static::getDataFromString($options->text));
        }
        elseif($type == 'data' && $options->data_key && $options->value_key){
            $vkey = static::getDataFromString($options->value_key);
            $content = e(static::$config->get('data.'.$options->data_key.'.'.$vkey));
        }
        elseif($options->data_access){
            $key = str_eval($options->data_access, array_merge(static::data(), static::parseTemplateData($options->data)), 0, '');
            $content = static::$config->get('data.'.$key);
        }
        elseif($type == 'template' || $options->template){
            $content = str_eval($options->template, array_merge(static::data(), static::parseTemplateData($options->data)), 0, '');
        }
