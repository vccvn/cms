Crazy.comments = {
    ...
    changeApproveStatus:function (status, elem) {
        ...
        cl(data)
        Crazy.ajax(this.urls.change_approve_url, "POST", data, function(rs){
            cl(rs.status);
        })
    }
};
...
$(function(){
    ...
    if(typeof comment_urls != 'undefined' && Crazy.getType(comment_urls) == 'object'){
        for (const key in comment_urls) {
            if (comment_urls.hasOwnProperty(key)) {
                const url = comment_urls[key];
                Crazy.comments.urls[key] = url;
            }
        }
    }
