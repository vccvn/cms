...
Order.list = {
    ...
    onStart:function(){
        ...
        $('.order-item-status-select').change(function(){
            ...
            Crazy.ajax(self.urls.change_status, "POST", data, function(rs){
        ...
        $('.view-order-detail').click(function(){
            ...
            showLoading();
            Crazy.ajax(self.urls.get_detail, "GET", {id:id}, function(rs){
                if(rs.status){
    ...
    showDetail:function(data){
        ...
        +'</div>';
        let content = '';
        data.total_pay = Crazy.number.currency(parseInt(data.total_money + data.ship_fee + data.vat));
        for (const key in self.fields) {
            if (data.hasOwnProperty(key) || key == "address") {
                ...
                if(self.data.hasOwnProperty(key) && self.data[key].hasOwnProperty(raw)){
                    ...
                    value = Crazy.number.currency(parseInt(raw));
                }
        ...
        if(items){
            ...
            for (const key in items) {
                if (items.hasOwnProperty(key)) {
                    let item = items[key];
                    item.final_price = Crazy.number.currency(parseInt(item.final_price));
                    item.total_price = Crazy.number.currency(parseInt(item.final_price*item.quantity));
                    
                    item.attrs = self.renderAttributes(item.attributes);
                    content += Crazy.str.eval(html_body, item);
                }
            }
            content+=html_footer;
            return content;
