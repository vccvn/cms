function FormSetting(selector) {
    ...
    this.remove = function(){
        ...
        Crazy.modal.confirm("Bạn có chắc là muốn xóa group này không?", function(stt){
            fsSstatus = false;
            $el.hide(400);
            var data = {
            ...
            showLoading();
            Crazy.ajax(FSetting.delete_url, "POST", data, function(res){
                if(res.status){
                    top.location.reload();
                }else{
                    hideLoading();
...
var FSetting = {
    ...
    save: function(success, fail){
        if(self.isSendingRequest){
            ...
            return Crazy.modal.alert("Đang xử lý yêu cầu");
        }
        ...
        Crazy.ajax(this.submit_url, "POST", data, function(rs) {
            self.isSendingRequest = false;
            if(rs.status){
        ...
        this.addon.find('.btn-add-group').on('click', function(){
            ...
            Crazy.modal.popup({
                ...
                inputs:{
                    ...
                    self.addon.before(Crazy.str.eval(self.template,data));
