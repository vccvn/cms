const CrazyGallery = function(selector){
    ...
    this.open = function(editor){
        $editor = editor;
        Crazy.modal.show('gallery-modal');
        if(!firstLoad){
            this.getResults();
            firstLoad = true;
        }
    ...
    this.getResults = function(){
        ...
        Crazy.ajax(url, "GET",data, function(rs){
            $loading.hide();
            if(rs.status){
        ...
        if(data != null && data.length){
            for (let i = 0; i < data.length; i++) {
                ...
                if(!$('#gallery-image-item-'+image.id).length){
                    $galleryImages.append(Crazy.str.eval(template, image));
                }
            }
            $results.show();
            $message.hide();
    ...
    this.prepand = function(image){
        $galleryImages.prepend(Crazy.str.eval(template, image));
        $results.show();
        $message.hide();
        $loading.hide();
        $results.stop().animate({
    ...
    this.remove = function(id){
        ...
        if (ans) {
            Crazy.ajax(deleteUrl, "POST", { ids: [id] }, function(rs) {
                if (rs.status) {
                    if (rs.data) {
    ...
    this.insert = function(){
        ...
        if(!$imageSelect.length){
            ...
            $imageSelect.each(function(i, img){
                var $im = $(img);
                content += "<img src=\""+$im.data('src')+"\" alt=\""+$im.data('filename')+"\">";
            });


            $editor.insertContent(content);
            Crazy.modal.hide("gallery-modal");
