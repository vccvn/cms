$(function(){
    const MyTheme = function(options) {
        ...
        this.active = id => {
            if(parseInt(id) == parseInt(self.current_theme)) return Crazy.modal.alert("Theme đã được kích hoạt!");
            ajaxRequest(this.urls.active, "POST", {id:id}, rs => {
                if(rs.status){
                    ...
                    Crazy.modal.alert("Đã kích hoạt thành công!");
                }
            })
        };
        ...
        this.load = (keywords, tab, page) => {
            ...
            Crazy.ajax(this.urls.search, "GET", data, function(rs){
                setTimeout(function(){
                    afterLoad();
                    self.showResults(rs);
                }, Crazy.number.rand(200, 1000));
                ...
                if(rs.data && rs.data.length){
                    ...
                    for (const key in rs.data) {
                        if (rs.data.hasOwnProperty(key)) {
                            ...
                            let tool = '';
                            if(theme.available){
                                tool = Crazy.str.eval(self.templates.available, theme);
                            }else{
                                tool = Crazy.str.eval(self.templates.pending, theme);
                            }
                            if(!theme.available || theme.privacy == "protected"){
                                tool += Crazy.str.eval(self.templates.delete, theme);
                            }
                            theme.tool = tool;
                            html += Crazy.str.eval(self.templates.item, theme);
                        }
                    }
        ...
        this.detail = id => {
            ajaxRequest(this.urls.detail, "GET", {id:id}, (rs) => {
                if(rs.status){
                    ...
                    var html = Crazy.str.eval(self.templates.detail, data);
                    let bns = [];
                    if(data.available){
                        btns = [
                            {
                ...
                if((type == 'array' || type == 'object') && gallery.length){
                    ...
                    for (const key in gallery) {
                        if (gallery.hasOwnProperty(key)) {
                            ...
                            html+=Crazy.str.eval(self.templates.gallery, {src:image});
                        }
                    }
                    html += '</div></div>';
                }
