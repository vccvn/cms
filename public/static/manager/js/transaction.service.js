...
Transaction.service = {
    ...
    onStart:function(){
        ...
        $('.view-transaction-detail').click(function(e){
            ...
            self.id = id;
            showLoading();
            Crazy.ajax(get_detail, "GET", {id:id}, function(rs){
                if(rs.status){
                    self.detail = rs.data;
    ...
    doAction:function(action, method, success){
        ...
        showLoading();
        Crazy.ajax(url, method, {id:id}, function(rs){
            hideLoading();
            if(rs.status){
                ...
                Crazy.modal.success(message, function(){
                    if(typeof success == "function"){
                        success(rs.data);
                    }
                });
            }else{
                Crazy.modal.warning("Đã có lỗi xảy ra vui lòng kiểm tra thông tin và thử lại sau giây lát");
            }
            
        }, function(err){
            hideLoading();
            
            Crazy.modal.error("Lỗi không xác dịnh. Vui lòng thử lại sau giây lát");
    ...
    changeCustomer:function(customer_id){
        var self = this;
        showLoading();
        Crazy.ajax(self.urls.get_service_options, "get", {customer_id:customer_id}, function(rs){
            if(rs.status && rs.data){
                Crazy.htmlSelect.changeOptions('ref_id', rs.data);
            }else{
                Crazy.htmlSelect.changeOptions('ref_id', ["Danh sách trống"]);
            }
            hideLoading();
        }, function(err){
            hideLoading();
    ...
    showDetail:function(data){
        ...
        data.amount = Crazy.number.currency(parseInt(data.amount));
        for (const key in self.fields) {
            if (data.hasOwnProperty(key)) {
                ...
                content += Crazy.str.eval(row_tpl, {label: label, value: value});
            }
        }
        let modalData = {
