...
Order.feedback = {
    ...
    onStart:function(){
        ...
        $(document).on('click', '.btn-resolve-feedback', function(){
            Crazy.ajax(self.urls.resolve, "POST",{id:self.id}, function(rs){
                if(rs.status){
                    Crazy.modal.success(rs.message);
                }
                else{
    ...
    changeCustomer:function(customer_id){
        ...
        Crazy.ajax(self.urls.get_order_options, "get", {customer_id:customer_id}, function(rs){
            if(rs.status && rs.data){
                Crazy.htmlSelect.changeOptions('order_id', rs.data);
            }else{
                Crazy.htmlSelect.changeOptions('order_id', ["Danh sách trống"]);
            }
            hideLoading();
        }, function(err){
            hideLoading();
