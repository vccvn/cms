...
const ajaxRequest = function(url, method, data, success, error){
    showLoading();
    Crazy.ajax(url, method, data, function(rs){
        hideLoading();
...
var Notice = {
    ...
    getNotices: function(){
        ...
        Crazy.ajax(this.get_notice_url, "get", {page:page, page_size:this.page_size}, function (rs){
            if(rs.status){
                self.first_open = !0;
                self.page = rs.page;
                ...
                if(rs.data) {
                    ...
                    for(var i = 0; i < rs.data.length; i++){
                        ...
                        html += Crazy.str.eval(template, notify);
                    }
                    ...
                    inp.addEventListener("change",function(){
                        ...
                        if(inpOnChanges.length){
                            inpOnChanges.forEach(element => {
                                let func = element.trim();
                                if(Crazy.func.check(func)){
                                    Crazy.func.call(func, [$this.value, $this]);
                                }
                            });
...
Crazy.location = {
    ...
    changeRegionID: function(value, text, el){
        if(value != this.regionID){
            this.regionID = value;
            Crazy.htmlSelect.deactive('district_id');
            Crazy.htmlSelect.deactive('ward_id');
            Crazy.htmlSelect.changeOptions('district_id', {0: "Chọn một"});
            Crazy.htmlSelect.changeOptions('ward_id', {0: "Chọn một"});
            if(value && value != "0"){
                ajaxRequest(this.urls.district_options, "get", {region_id: value}, function(res){
                    if(res.status){
                        Crazy.htmlSelect.changeOptions('district_id', res.data);
                    }
                });
            }
        }
        ...
        if(value != this.districtID){
            ...
            if(value && value != "0"){
                ajaxRequest(this.urls.ward_options, "get", {district_id: value}, function(res){
                    if(res.status){
                        Crazy.htmlSelect.changeOptions('ward_id', res.data);
                    }
                });
            }
        }
...
jQuery(document).ready(function(){
                ...
                if(onc){
                    ...
                    if(oncbs.length>1){
                        oncbs.forEach(element => {
                            let func = element.trim();
                            if(Crazy.func.check(func)){
                                Crazy.func.call(func, [self, fs]);
                            }
                        });
                        
                    }
                    else if(Crazy.func.check(onc)){
                        Crazy.func.call(onc, [self, fs]);
                    }
    ...
    if (typeof window.locationInit == "function") {
        ...
        if(typeof location_data == 'object'){
            Crazy.location.urls = location_data.urls || {};
        }
    }
    ...
    if (typeof window.postsInit == "function") {
        ...
        if(typeof posts_data == 'object'){
            Crazy.posts.urls = posts_data.urls || {};
        }
    }
