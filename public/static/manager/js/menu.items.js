$(function(){
    const MenuItem = function(options) {
        ...
        this.enableSort = function(){
            Crazy.func.call("Crazy.nestable.add", ['#crazy-menu-item-list']);
        };
        this.disableSort = function(){
            Crazy.func.call("Crazy.nestable.remove", ['#crazy-menu-item-list']);
        };

        this.sortCallback = function(l, e){
            var items = $(l).nestable('serialize');
            Crazy.ajax(self.urls.sort, "POST", {items:items}, function(rs){
                if(rs.status){
        ...
        this.delete = function(id){
            Crazy.modal.confirm("bạn có chắc chắn muốn xóa item này?", function(status){
                if(status){
                    showLoading();
        ...
        this.showForm = function(prefix){
            Crazy.modal.show('menu-item-modal-'+prefix, null, function(){
                // 
        ...
        this.showFormData = function(data){
            ...
            if(self.selectTypes.indexOf(data.type) >= 0){
                if(data.type != 'post_category'){
                    Crazy.htmlSelect.active(prefix+'-'+data.type+"_id", data.ref_id);
                }else{
                    // cl (prefix+'-'+data.type+"_id-dynamic-"+data.dynamic_id);
                    Crazy.htmlSelect.active(prefix+'-'+data.type+"_id-dynamic-"+data.dynamic_id, data.ref_id);
                }
        ...
        this.resetForm = function(form, data) {
            ...
            if(self.selectTypes.indexOf(data.type) >= 0){
                if(data.type != 'post_category'){
                    Crazy.htmlSelect.active(prefix+'-'+data.type+"_id", 0);
                }else{
                    Crazy.htmlSelect.active(prefix+'-'+data.type+"_id-dynamic-"+data.dynamic_id, 0);
                }
            }
            else if(type == 'route'){
                ...
                if(rs.status){
                    ...
                    if(self.id && data.id == self.id){
                        ...
                        if(itemText.length){
                            ...
                            $('#crazy-menu-item-list').nestable('replace', {id:rs.data.id,content:Crazy.str.eval(self.template, data)});
                        ...
                        $('#crazy-menu-item-list').nestable('add', {id:rs.data.id,content:Crazy.str.eval(self.template, data)});
                        var item = $('#crazy-menu-item-list li.dd-item[data-id="'+data.id+'"]');
                        if(item.length){
