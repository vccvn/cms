    ...
    const TeamManager = function(options) {
        ...
        this.onStart = function() {
            ...
            $(document).on('click', '.btn-delete-member', function(){
                ...
                Crazy.modal.confirm(message, function(stt){
                    if(stt){
                        showLoading();
                        Crazy.ajax(self.urls.delete_member, "DELETE", {id:id}, function(rs){
                            hideLoading();
                            if(rs.status){
                                item.remove();
                            }
                            Crazy.modal.show('team-modal');
            ...
            $(document).on('click', '.btn-edit-member', function(){
                showLoading();
                Crazy.ajax(self.urls.member_detail, "get", {id:$(this).data('id')}, function(rs){
                    hideLoading();
                    if(rs.status){
                    ...
                    hideLoading();
                    Crazy.modal.error("Lỗi không xác định. Vui lòng thử lại sau giay lát", null, function(){
                        self.showForm();
                    });
                });
                return false;
        ...
        this.getDetail = function(id, callback){
            this.id = id;
            showLoading();
            Crazy.ajax(this.urls.detail, "GET", {id:id}, function(rs){
                hideLoading();
                if(rs.status){
        ...
        this.showDetail = function(data){
            ...
            if(data.members){
                data.members.map(function(member){
                    ...
                    members += Crazy.str.eval(self.template, member);
        ...
        this.showForm = function(){
            Crazy.modal.show('team-member-modal', null, function(){
                Crazy.modal.show('team-modal');
            });
        };
        ...
        this.showFormData = function(data){
            ...
            Crazy.htmlSelect.ajaxSearch('member_id', form.member_id, function(){
                Crazy.htmlSelect.active('member_id', form.member_id);
            });
