function OrderItem(selector){
    ...
    this.init = function(){
        ...
        $el.on('click', '.btn-add-product-item',function(){
            ...
            Crazy.ajax(self.addProductUrl, "GET", data, function(res){
                if(res.status){
                    self.nextIndex++;
...
Order.form = {
    ...
    changeRegionID: function(value, text, el){
        if(value != this.regionID){
            this.regionID = value;
            Crazy.htmlSelect.deactive('billing_district_id');
            Crazy.htmlSelect.deactive('billing_ward_id');
            Crazy.htmlSelect.changeOptions('billing_district_id', {0: "Chọn một"});
            Crazy.htmlSelect.changeOptions('billing_ward_id', {0: "Chọn một"});
            if(value && value != "0"){
                ajaxRequest(this.urls.district_options, "get", {region_id: value}, function(res){
                    if(res.status){
                        Crazy.htmlSelect.changeOptions('billing_district_id', res.data);
                    }
                });
            }
        }
        ...
        if(value != this.districtID){
            ...
            if(value && value != "0"){
                ajaxRequest(this.urls.ward_options, "get", {district_id: value}, function(res){
                    if(res.status){
                        Crazy.htmlSelect.changeOptions('billing_ward_id', res.data);
                    }
                });
            }
        }
    ...
    changeShippingRegionID: function(value, text, el){
        if(value != this.shippingRegionID){
            ...
            Crazy.htmlSelect.deactive('shipping_district_id');
            Crazy.htmlSelect.deactive('shipping_ward_id');
            Crazy.htmlSelect.changeOptions('shipping_district_id', {0: "Chọn một"});
            Crazy.htmlSelect.changeOptions('shipping_ward_id', {0: "Chọn một"});
            if(value && value != "0"){
                ajaxRequest(this.urls.district_options, "get", {region_id: value}, function(res){
                    if(res.status){
                        Crazy.htmlSelect.changeOptions('shipping_district_id', res.data);
                    }
                });
            }
        }
    ...
    changeShippingDistrictID: function(value, text, el){
        if(value != this.shippingDistrictID){
            ...
            if(value && value != "0"){
                ajaxRequest(this.urls.ward_options, "get", {district_id: value}, function(res){
                    if(res.status){
                        Crazy.htmlSelect.changeOptions('shipping_ward_id', res.data);
                    }
                });
            }
        }
