...
Profile.experiences = {
    ...
    showForm: function(){
        Crazy.modal.show("experience-modal");
        this.checkForm();
    },
    showFormData:function(data){
        ...
        Crazy.htmlSelect.active("org_id", form.org_id);
        
        Crazy.htmlSelect.active("started-day", form.started.day);
        Crazy.htmlSelect.active("started-month", form.started.month);
        Crazy.htmlSelect.active("started-year", form.started.year);

        Crazy.htmlSelect.active("finished-day", form.finished.day);
        Crazy.htmlSelect.active("finished-month", form.finished.month);
        Crazy.htmlSelect.active("finished-year", form.finished.year);
    ...
    addItem:function(data){
        $('.crazy-timeline .timeline-body').append(Crazy.str.eval(this.template, data));
    ...
    delete: function(id){
        ...
        let item = $('#crazy-timeline-item-'+id);
        if(item.length){
            Crazy.modal.confirm('Bạn có chắc chắn muốn xóa '+item.data('title')+' không?', function (yes) {
                if(yes){
                    Crazy.ajax(self.urls.delete_url, "DELETE", {id:id},function(rs){
                        if(rs.status){
                            item.remove();
