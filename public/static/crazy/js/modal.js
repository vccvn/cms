...
Crazy.modal = {
                            ...
                            if (key == 'text') {
                                ...
                                if(typeof val == 'function'){
                                    ...
                                    button.addEventListener('click', function(e){
                                        Crazy.func.call(this.getAttribute('data-click'));
                                    },false);
                                }
                            } else {
    ...
    popup:function(options, doneCallback, showCallback, hideCallback){
        ...
        if(Crazy.getType(options) == 'object'){
            ...
            if(typeof options.inputs != 'undefined' && options.inputs){
                var t = Crazy.getType(options.inputs);
                if(t == "object"){
            ...
            if(typeof options.validate != "undefined" && Crazy.getType(options.validate) == "object"){
                ...
                if (inplist.hasOwnProperty(name)) {
                    ...
                    for (const prop in inp) {
                        if (inp.hasOwnProperty(prop)) {
                            ...
                            if(p != 'type' && p != 'append' && p != 'prepend' && p != 'value' && p != 'label' && (tv == 'string' || tv == 'number')){
                                ...
                                }else if(Crazy.func.check(val)){
                                    events[p.substr(1)] = function(e){
                                        Crazy.func.call(val,[e]);
                                    }
                                }
                            }
                        }
                    ...
                    if(type == 'textarea') $inp = '<'+type+props+'>'+(value)+'</'+type+'>';
                        ...
                        $inp = '<div class="custom-file">' +
                            '<input type="file" '+props+' data-on-change="Crazy.modal.onPopupFile">' +
                            '<label class="custom-file-label" for="'+id+'">'+(value.length?Crazy.str.htmlentities(value):'Chọn file')+'</label>' +
                        '</div>';
                    }
                    else{
                        $inp = '<input type="'+type+'"'+props+' value="'+Crazy.str.htmlentities(value)+'">';
                    }
                    ...
                    if(type == "hidden"){
                        html+= Crazy.str.eval($inp, {input_class:input_class});
                    }
                    else{
                        html+=Crazy.str.eval(formGroup, {
                            label_class: label_class + " col-form-label",
                            input_class:input_class,
                            wrapper_class:wrapper_class,
                            label:label,
                            id:id,
                            input:Crazy.str.eval($inp, {input_class:input_class})
                        });
                    }
...
$(function() {
    ...
    if ($('.modal').length > 0) {
        $('#confirm-modal .btn-confirm-answer').click(function() {
            var $this = $(this);
            Crazy.modal.hide('confirm-modal');
            if ($this.hasClass('yes')) {
                Crazy.modal.answer(true);
            } else {
                Crazy.modal.answer(false);
            }
        });
        ...
        $('#popup-modal #crazy-popup-form').submit(function(e) {
            ...
            if(files.length){
                for (let i = 0; i < files.length; i++) {
                    const inp = files[i];
                    if(typeof Crazy.modal.popupFiles[inp.name] != "undefined"){
                        data.push({
                            name: inp.name,
                            value: Crazy.modal.popupFiles[inp.name]
                        })
                    }
                }
            }
            Crazy.modal.popupDone(data);
            return false;
        });

        $('.modal').on('hidden.bs.modal', function() {
            Crazy.modal.is_on = false;
            var $this = this;
            setTimeout(function(){
                if (typeof Crazy.modal.hide_callback == 'function') {
                    Crazy.modal.hide_callback($this, Crazy.modal);
                }
                
                Crazy.modal.last_open = Crazy.modal.current_open;
                Crazy.modal.current_open = null;
            
                Crazy.modal.hide_callback = null;
            }, 200);
        ...
        $('.modal').on('show.bs.modal', function(e) {
            var $this = this;
            Crazy.modal.is_on = true;
            
            setTimeout(function(){
                Crazy.modal.current_open = $($this).attr('id');
                if (typeof Crazy.modal.show_callback == 'function') {
                    Crazy.modal.show_callback($this, Crazy.modal);
                }
                Crazy.modal.show_callback = null;
            }, 200);
