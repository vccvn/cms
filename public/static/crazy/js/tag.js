function InputTag(selector){
    ...
    this.ajaxSearch = function(keyword){
        ...
        this.showLoading();
        Crazy.ajax(self.searchUrl, "GET", data, function(rs){
            self.hideLoading();
                    ...
                    for (const index in rs.data) {
                        if (rs.data.hasOwnProperty(index)) {
                            ...
                            if(!self.exists(value, text)){
                                results += "<a href=\"javascript:;\" class=\"nav-link search-result-item search-item-"+value+"\" data-value=\""+value+"\" data-text=\""+Crazy.str.htmlentities(text)+"\">"+Crazy.str.htmlentities(text)+"</a>";
                            }
    ...
    this.createTags = function(){
        ...
        this.showLoading();
        Crazy.ajax(self.createUrl, "POST", data, function(rs){
            if(rs.status){
                if(rs.data){
                    let data = rs.data;
                    if(Crazy.getType(rs.data) == 'object') data = [rs.data];
                ...
                if(rs.errors){
                    ...
                    Crazy.modal.error(messages.join('<br />'));
                }else{
