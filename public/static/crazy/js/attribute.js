function ProductAttribute(selector){
        ...
        $el.on('click', '.btn-add-attribute-value', function(){
            ...
            self.addValuePopup(attribute_id, value_type, label, function(rs){
                ...
                if($this.hasClass('ref-checklist')){
                    ...
                    $el.find('.attribute-' +attribute_id+ '-wrapper .btn-add-value-block').before(Crazy.str.eval(tpl, {text:text, value: value}));
            ...
            btnDone: "Thêm",
                ...
                function(ers){
                    if(ers.errors){
                        ...
                        Crazy.modal.error(messages.join('<br />'), failCallback);
                    }
                });
            }
        });
    ...
    this.addAttributeValue = function(data, success, fail){
        ...
        showLoading();
        Crazy.ajax(addValueUrl, "POST", data, function(rs){
            hideLoading();
            if(rs.status){
                success(rs);
            }else if(fail){
